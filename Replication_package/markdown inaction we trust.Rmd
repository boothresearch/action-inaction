...
data1 <- read.csv(file.path("data", "inactionwetrust_study1.csv"),
                  na.strings = c("", "NA"))

# data cleaning
## remove variable descriptions + practice data
data1 <- data1[-c(1:1), ]
data1 <-  data1 [, c("preference",
                     "competence",
                     "norms",
                     "Q55",
                     "check1",
...
dplot1 <- data1 %>% select(preference, competence, normative, regret) %>%
  ...
  gather()
dplot1$key <- factor(dplot1$key,
                     levels = c("Preference", "Competence", "Norms", "Regret"))

dplot1  %>% ggplot(aes(value)) +
  facet_wrap( ~ key, scales = "free", ) + geom_histogram(binwidth = 1) +
  theme_apa() +
...
Sa1preferenceplot
ggsave(file.path("output", "Study1a-preference.png"),
       dpi = 600,
       scale = 1.7)
...
Sa1competenceplot
ggsave(file.path("output", "Study1a-competence.png"),
       dpi = 600,
       scale = 1.7)
...
Sa1normativeplot
ggsave(file.path("output", "Study1a-norms.png"),
       dpi = 600,
       scale = 1.7)
...
Sa1regretplot
ggsave(file.path("output", "Study1a-regret.png"),
       dpi = 600,
       scale = 1.7)
...
data1b <- read.csv(file.path("data", "inactionwetrust_study1b.csv"))
...
dplot1b <- data1b %>% select(preference, competence, normative, regret) %>%
  ...
  gather()
dplot1b$key <- factor(dplot1b$key,
                      levels = c("Preference", "Competence", "Norms", "Regret"))
...
S1bpreferenceplot
ggsave(file.path("output", "Study1b-preference.png"),
       dpi = 600,
       scale = 1.7)
...
S1bcompetenceplot
ggsave(file.path("output", "Study1b-competence.png"),
       dpi = 600,
       scale = 1.7)
...
S1bnormativeplot
ggsave(file.path("output", "Study1b-norms.png"),
       dpi = 600,
       scale = 1.7)
...
S1bregretplot
ggsave(file.path("output", "Study1b-regret.png"),
       dpi = 600,
       scale = 1.7)
